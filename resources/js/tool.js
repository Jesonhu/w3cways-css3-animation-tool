    ...
    */
	...
	Tab: function() {
		...
		$("#tab_nav li").click(function() {
			...
			if ($index === toggleIndex) {
				...
				if (!isFoldSideMenu) {
					$('.canvas').animate({ left: '30px' }, 500, 'linear');
					$('.ani').animate({ left: '50px' }, 500, 'linear');
					$('.mod_tab_content').animate({ width: 0 }, 500, 'linear', function() {
						$('.mod_tab_content').css('display', 'none');
						$('.canvas').css('left', '30px');
					});
				} else {
					$(this).text('《');
					$('.canvas').animate({ left: '390px' }, 500, 'linear');
					$('.ani').animate({ left: '390px' }, 500, 'linear');
					$('.mod_tab_content').css({display: 'block'}).animate({ width: '350px' }, 500, 'linear');
				}
				isFoldSideMenu = !isFoldSideMenu;
				return;
			} else {
				isFoldSideMenu = false;
				$('.canvas').animate({ left: '390px' }, 500, 'linear');
				$('.ani').animate({ left: '390px' }, 500, 'linear');
				$('.mod_tab_content').css({display: 'block'}).animate({ width: '350px' }, 500, 'linear');
			}
	...
	animateDemo: function() {
		...
		$(".butt").click(function() {
			...
			//显示代码
			$.getJSON("./resources/other/animate.json", function(data) {
				var cssCode = data[0];
